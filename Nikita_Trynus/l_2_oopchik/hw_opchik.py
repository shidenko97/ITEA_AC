...
needed_dict = dict(
    zip(
        list_of_metric_summary[0]["cpc"].keys(),
        [None] * len(list_of_metric_summary[0]["cpc"].keys()),
    )
)
...
FacebookInsight = type(
    "FacebookInsight",
    (BaseInsight,),
    {"dimensions_dict": None, "dimensions": None},
)
GoogleInsight = type("GoogleInsight", (BaseInsight,), {"actions": None})
TwitterInsight = type(
    "TwitterInsight", (BaseInsight,), {"first_date": None, "last_date": None}
)
SnapchatInsight = type(
    "SnapchatInsight", (BaseInsight,), {"weight": None, "type": None}
)
