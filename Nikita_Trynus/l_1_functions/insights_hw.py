...
def calculating(string):
    ...
    sum_ = list(
        map(
            lambda x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11: x1[string]
            + x2[string]
            + x3[string]
            + x4[string]
            + x5[string]
...
def formulas(key_for_formula):
    a = {
        1: lambda sum_, sum_level, sum_general, period: (
            sum_ * sum_level / sum_general
        )
        / period,
        2: lambda sum_, sum_level, sum_general, period: (
            sum_ * sum_level ** 2 / sum_general
        )
        / period,
        3: lambda sum_, sum_level, sum_general, period: (
            sum_level / sum_general
        )
        / period,
        4: lambda sum_, sum_level, sum_general, period: (
            sum_level * sum_general / 100
        )
        / period,
    }
...
def work_with_apis(list_of_insights):
    ...
    for i in list_of_insights:
        if not i.get("metric_sums") is None:
            list_of_metrics_and_periods_api.append(
                (i["metric_sums"], i["period"], i["api"])
            )
    ...
    for lis, period, api in list_of_metrics_and_periods_api:
        for metrics in lis:
            try:
                ...
                print(
                    err,
                    " Why on earth are you doing this shit, crazy motherfucker!?!?!?!",
                )
...
if __name__ == "__main__":
    # 1 exercise
    new_insights = remove_by_value(
        insights, "period count total_count page_id link status days_in_data"
    )
    ...
    dict_of_objectives = dict(
        zip(list(range(len(list_of_objectives))), list_of_objectives)
    )
    unique = set(list_of_objectives)

    # 5 exercise
    list_of_campaign_id = get_all(insights, [], "campaign_id")
    dict_of_campaign_id = dict(
        zip(list(range(len(list_of_campaign_id))), list_of_campaign_id)
    )
